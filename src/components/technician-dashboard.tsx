...
import { Loader2, ListTodo, Wrench, CheckCircle, HardHat } from 'lucide-react';
...
export default function TechnicianDashboard() {
  const { user } = useAuth();
  const [workOrderStats, setWorkOrderStats] = React.useState<WorkOrderStats | null>(null);
  const [installationOrderStats, setInstallationOrderStats] = React.useState<InstallationOrderStats | null>(null);
    ...
    if (user?.role === 'tecnico') {
      ...
      Promise.all([
        ...
        getInstallationOrders(user),
          ...
          const newWorkOrderStats: WorkOrderStats = workOrders.reduce(
            (acc, order) => {
          ...
          setWorkOrderStats(newWorkOrderStats);
          
          const newInstallationOrderStats: InstallationOrderStats = installationOrders.reduce(
            (acc, order) => {
              if (order.estado === 'pendiente') acc.pending += 1;
          ...
          setInstallationOrderStats(newInstallationOrderStats);
  ...
  if (isLoading || !workOrderStats || !installationOrderStats) {
    return (
