...
export const ClientSchema = z.object({
  ...
  codIdSujeto: z.string().min(1, 'Código de sujeto es requerido.'),
  ...
  valOperacion: z.coerce.number({invalid_type_error: "Debe ser un número"}).positive('Valor de operación debe ser positivo.'),
  valorPago: z.coerce.number({invalid_type_error: "Debe ser un número"}).nonnegative('Valor de pago no puede ser negativo.'),
  fecVencimiento: z.date({ required_error: 'Fecha de vencimiento es requerida.' }),
  valorVencido: z.coerce.number({invalid_type_error: "Debe ser un número"}).nonnegative('Valor vencido no puede ser negativo.'),
